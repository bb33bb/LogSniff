...
LRESULT SyntaxTextView::WndSubProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    PWIN_PROC pfnOldProc = param.mOldWndProc;
    switch (msg) {
        case WM_NOTIFY:
            {
                NMHDR *header = (NMHDR *)lp;
                if (header && (header->hwndFrom != NULL))
                {
                    for (set<SyntaxTextView *>::const_iterator it = param.mSyntaxSet.begin() ; it != param.mSyntaxSet.end() ; it++)
                    {
                        SyntaxTextView *ptr = *it;

                        if (ptr->GetWindow() == header->hwndFrom)
                        {
...
bool SyntaxTextView::CreateView(HWND parent, int x, int y, int cx, int cy) {
    ...
    if (IsWindow(m_hwnd))
    {
        ...
        if (IsWindow(m_parent))
        {
            ...
            {
                ProcHookParam param;
                param.mOldWndProc = (PWIN_PROC)SetWindowLongPtrA(m_parent, DWLP_DLGPROC, (LONG_PTR)WndSubProc);
                param.mSyntaxSet.insert(this);
                msWinProcCache[m_parent] = param;
            } else {
